...
def get_file_indices(image_folder):
    return list(map(lambda file_name: file_name.split('.')[0], os.listdir(image_folder)))
...
def xml_to_coco(xml_folder, image_folder, xml_category, filename_to_id):
    scale = 0.64
    file_indices = get_file_indices(image_folder)
    coco_data = {
        "images": [],
        "annotations": [],
        "categories": []
...
def main(xml_folder, image_folder, output_json_folder, output_yolo_folder, model_path, batch_size):
    ...
    for i in tqdm(range(0, len(image_paths), batch_size)):
        ...
        for res, real_path in zip(results, batch_paths):
            res.path = real_path
        all_results.extend(results)

    if args.save:
        os.makedirs(output_yolo_folder, exist_ok=True)
        save_inference_results(image_folder, all_results, output_yolo_folder)

    os.makedirs(output_json_folder, exist_ok=True)
    yolo_annotations = yolo_to_coco_annotations(all_results, image_folder, filename_to_id)
    with open(os.path.join(output_json_folder, 'yolo_result.json'), 'w') as f:
        json.dump(yolo_annotations, f, cls=MyEncoder)

    xml_coco_data = xml_to_coco(xml_folder, image_folder, xml_category, filename_to_id)
    with open(os.path.join(output_json_folder, 'xml_result.json'), 'w') as f:
        json.dump(xml_coco_data, f, cls=MyEncoder)

    coco_gt = COCO(os.path.join(output_json_folder, 'xml_result.json'))
    coco_dt = coco_gt.loadRes(os.path.join(output_json_folder, 'yolo_result.json'))
    ...
    with open(os.path.join(args.base_folder, 'yolo_mAP.txt'), 'w') as f:
        print(coco_eval.stats[0], file=f)
    with open(os.path.join(args.base_folder, 'yolo_mAP50.txt'), 'w') as f:
        print(coco_eval.stats[1], file=f)
    with open(os.path.join(args.base_folder, 'yolo_mAP75.txt'), 'w') as f:
        print(coco_eval.stats[2], file=f)
...
parser.add_argument('--base_folder', type=str, help='Base dir')
parser.add_argument('--xml_folder', type=str, help='XML dir')
parser.add_argument('--model_path', type=str, default=os.path.join(PROJECT_DIR, 'pretrained', 'evaluation', 'yolo', 'best.pt'), help='Model path')
parser.add_argument('--save', action='store_true', default=False, help='Save')
args = parser.parse_args()

novel_categories = ['windmill', 'airport', 'chimney', 'dam', 'trainstation']

if __name__ == '__main__':
    model_path = args.model_path
    xml_folder = args.xml_folder
    image_folder = os.path.join(args.base_folder, 'image')
    output_json_folder = os.path.join(args.base_folder, 'json')
    output_yolo_folder = os.path.join(args.base_folder, 'yolo')

    main(xml_folder, image_folder, output_json_folder, output_yolo_folder, model_path, batch_size=128)
