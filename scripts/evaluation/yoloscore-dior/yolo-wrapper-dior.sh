        ...
        for config in "${CONFIGS[@]}"; do
            for seed in "${SEEDS[@]}"; do
                for ckpt in "${CKPTS[@]}"; do
                    ...
                    python yolo-dior.py \
                        --base_folder $DSP_PROJECT_DIR/outputs/$config/novel/run-$run_id/$k_shot-shot/shuffle_seed-$seed/checkpoint-$ckpt \
                        --xml_folder /path/to/DIOR-VOC/Annotations/Horizontal_Bounding_Boxes # Replace with your actual path
                done
            done
        done
    done
